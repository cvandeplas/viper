...
class Metascan(Module):
    ...
    def submit(self):
        headers = {'filename': __sessions__.current.file.name.encode('utf-8')}
    ...
    def poll_result(self, data_id):
        while True:
            ...
            if r_json['scan_results']['progress_percentage'] < 100:
                # need to poll again
                time.sleep(0.2)
            elif r_json['scan_results']['progress_percentage'] == 100:
                return r_json
            else:
                self.log('error', 'Unexpected result in poll request for data_id={}'.format(data_id))
                self.log('error', '{}'.format(r_json))
                return
        ...
        try:
            ...
            for scanner in result['scan_results']['scan_details']:
                ...
                if item['scan_result_i'] == 1 or self.args.verbose:
                    report.append([scanner, string_clean(item['threat_found']), item['def_time'].replace('T00:00:00Z', '').replace('T00:00:00', '')])
            ...
            self.log('error', "Error while connecting to metascan server.")
            import traceback
